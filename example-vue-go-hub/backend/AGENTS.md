...
- **[BE-6]** No business logic in handlers. Handlers parse input, call a repository, and return a response.
...
## Commands (single-package -- prefer these for faster feedback)
```bash
go vet ./internal/auth/                        # Vet one package
...
- Don't put business logic in handlers -- delegate to `internal/repository/`
